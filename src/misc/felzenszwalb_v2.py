...
# ---------- Downsampling & Supervoxel Embedding ----------

def approximate_supervoxels(points, voxel_size=0.05):
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(points)
    down_pcd = pcd.voxel_down_sample(voxel_size=voxel_size)
    down_points = np.asarray(down_pcd.points)

    # Assign full-res points to nearest downsampled (voxel center)
    nn = NearestNeighbors(n_neighbors=1).fit(down_points)
    indices = nn.kneighbors(points, return_distance=False).flatten()
    return down_points, indices

def compute_weighted_down_embeddings(points, embeddings, down_points, neighbor_k=30):
    nn = NearestNeighbors(n_neighbors=neighbor_k).fit(points)
    neighbor_dists, neighbor_idxs = nn.kneighbors(down_points, return_distance=True)

    weighted_embeddings = np.zeros((len(down_points), embeddings.shape[1]), dtype=np.float32)
    eps = 1e-6

    for i in tqdm(range(len(down_points)), desc="Computing weighted down embeddings"):
        dists = neighbor_dists[i] + eps
        weights = 1.0 / dists
        weights /= weights.sum()

        emb = embeddings[neighbor_idxs[i]]
        weighted_embeddings[i] = np.sum(emb * weights[:, None], axis=0)

    # Normalize embeddings to unit length
    norms = np.linalg.norm(weighted_embeddings, axis=1, keepdims=True) + 1e-8
    weighted_embeddings /= norms
...
def segment_point_cloud(points, embeddings, voxel_size=0.1, k_graph=30, seg_k=0.1,
                        min_size=100, k_label=5, batch_size=5000, alpha=0.5, beta=0.5, neighbor_k_embed=30):
    down_points, voxel_indices = approximate_supervoxels(points, voxel_size=voxel_size)
    down_embeddings = compute_weighted_down_embeddings(points, embeddings, down_points, neighbor_k=neighbor_k_embed)

    print(f"Original points: {len(points)}, Downsampled: {len(down_points)}")

    down_labels = efficient_graph_segmentation(
        down_points, down_embeddings,
        k=k_graph, seg_k=seg_k,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Segment a point cloud using geometric+embedding graph segmentation with supervoxel refinement.")
    ...
    parser.add_argument("--alpha", type=float, default=0.1, help="Weight for cosine geometric distance.")
    parser.add_argument("--beta", type=float, default=0.9, help="Weight for cosine embedding distance.")
    parser.add_argument("--voxel_size", type=float, default=0.05, help="Voxel size for supervoxel approximation.")
    parser.add_argument("--neighbor_k_embed", type=int, default=30, help="Number of neighbors for weighted embedding.")
    args = parser.parse_args()

    data = np.load(args.input)
    points = data['points']
    colors = data['colors']
    embeddings = data['embeddings']  # assumed normalized
    ...
    cluster_labels = segment_point_cloud(
        ...
        neighbor_k_embed=args.neighbor_k_embed
    )
